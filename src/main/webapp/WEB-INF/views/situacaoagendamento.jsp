...
<div>
    <div class="headerContainer">
        <h4 class="headerTitle">Situação Agendamento</h4>
    ...
    <form:form class="form-cadastro my-2" id="form-cadastro" modelAttribute="tblSituacaoAgendamento" action="${pageContext.request.contextPath}/situacaoagendamento/salvar" method="POST">
...
<div>
    <table id="tabelaDados" class="table table-bordered table-hover table-responsive my-3">
        ...
        <tbody>
        <c:forEach var="situacaoagendamento" items="${listarSituacaoAgendamentos}">
            <tr>
                <td class="cel-img-tabela-clientes">
                    <form action="${pageContext.request.contextPath}/situacaoagendamento/delete/${situacaoagendamento.id}" method="POST">
                        <img src="${pageContext.request.contextPath}/img/icones tabela clientes/escrever-999.png" class="icones-tabela icone-tabela-editar mx-2" onclick="visualizarSituacaoAgendamento('${situacaoagendamento.id}', '${situacaoagendamento.nome}'); return false;" title="Editar">
                        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                        <a href="#" onclick="confirmarExclusao(event, '${pageContext.request.contextPath}/situacaoagendamento/delete/${situacaoagendamento.id}')">
                ...
                <td><c:out value="${situacaoagendamento.nome}" /></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
